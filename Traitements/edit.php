...
if (isset($_POST['edit'])) {
    ...
    if (!empty([$nom && $prix && $quantite && $quantite && $type && $description])) {
        ...
        $req = $bdd->prepare('UPDATE medicament SET nom =?, prix =?, quantite =?, type =?, description =?, date =?, img =? WHERE id = "'.$id.'"');

        $req->execute(array($nom, $prix, $quantite, $type, $description, $date, $ph_name));

        //si les donnees on été bien inserrées on redirige avec le message suivant
        header('Location: ../index.php?reg_err=success_edit'); die();
    } else {
        header('Location: ../index.php?reg_err=err_r_edit'); die();
    }
